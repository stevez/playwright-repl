...
// ─── Debugger Domain (breakpoints, stepping) ──────────────────────────────────

let debugPausedCallFrameId: string | null = null;

async function debugEnable(): Promise<void> {
...
async function debugDisable(): Promise<void> {
  debugPausedCallFrameId = null;
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
...
async function debugSetBreakpoint(url: string, lineNumber: number): Promise<string> {
  const targetId = await ensureSelfAttached();
  return new Promise((resolve, reject) => {
    chrome.debugger.sendCommand(
      { targetId }, 'Debugger.setBreakpointByUrl', { url, lineNumber },
      (result: any) => {
        if (chrome.runtime.lastError) reject(new Error(chrome.runtime.lastError.message));
...
async function debugRemoveBreakpoint(breakpointId: string): Promise<void> {
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
    chrome.debugger.sendCommand(
      { targetId }, 'Debugger.removeBreakpoint', { breakpointId },
      () => {
        if (chrome.runtime.lastError) reject(new Error(chrome.runtime.lastError.message));
        else resolve();
      },
...
async function debugResume(): Promise<void> {
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
...
async function debugStepOver(): Promise<void> {
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
...
async function debugStepInto(): Promise<void> {
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
...
async function debugStepOut(): Promise<void> {
  const targetId = await ensureSelfAttached();
  await new Promise<void>((resolve, reject) => {
...
async function debugEvalInScope(expression: string): Promise<any> {
  const targetId = await ensureSelfAttached();
  const method = debugPausedCallFrameId ? 'Debugger.evaluateOnCallFrame' : 'Runtime.evaluate';
  const params: Record<string, unknown> = {
    expression, awaitPromise: true, returnByValue: true, generatePreview: true,
  };
  if (debugPausedCallFrameId) params.callFrameId = debugPausedCallFrameId;
  else params.replMode = true;
  return new Promise((resolve, reject) => {
    chrome.debugger.sendCommand({ targetId }, method, params, (result: any) => {
      if (chrome.runtime.lastError) reject(new Error(chrome.runtime.lastError.message));
      else if (result?.exceptionDetails) reject(new Error(result.exceptionDetails.exception?.description ?? 'Error'));
      else resolve(result?.result?.value);
    });
  });
}

// Listen for Debugger.paused / Debugger.resumed events and broadcast to offscreen → bridge
chrome.debugger.onEvent.addListener((source, method, params: any) => {
  if (source.targetId !== selfTargetId) return;
  if (method === 'Debugger.paused') {
    if (params.callFrames?.length > 0) {
      debugPausedCallFrameId = params.callFrames[0].callFrameId;
      ...
      // Broadcast to offscreen → bridge → VS Code
  ...
  if (method === 'Debugger.resumed') {
    debugPausedCallFrameId = null;
    chrome.runtime.sendMessage({ type: 'debug-resumed' }).catch(() => {});
  }
});
...
async function handleBridgeCommand(msg: {
  ...
  if (cmd === 'debug-enable') {
    await debugEnable();
    return { text: 'Debugger enabled', isError: false };
  }
  if (cmd === 'debug-disable') {
    await debugDisable();
    return { text: 'Debugger disabled', isError: false };
  }
  if (cmd.startsWith('debug-set-breakpoint ')) {
    const parts = cmd.split(' ');
    const line = parseInt(parts.pop()!, 10);
    const url = parts.slice(1).join(' ');
    const id = await debugSetBreakpoint(url, line);
    return { text: id, isError: false };
  }
  if (cmd.startsWith('debug-remove-breakpoint ')) {
    await debugRemoveBreakpoint(cmd.slice('debug-remove-breakpoint '.length));
    return { text: 'Breakpoint removed', isError: false };
  }
  if (cmd === 'debug-resume') {
    await debugResume();
    return { text: 'Resumed', isError: false };
  }
  if (cmd === 'debug-step-over') {
    await debugStepOver();
    return { text: 'Stepped over', isError: false };
  }
  if (cmd === 'debug-step-into') {
    await debugStepInto();
    return { text: 'Stepped into', isError: false };
  }
  if (cmd === 'debug-step-out') {
    await debugStepOut();
    return { text: 'Stepped out', isError: false };
  }
  if (cmd.startsWith('debug-eval ')) {
    const expr = cmd.slice('debug-eval '.length);
    const value = await debugEvalInScope(expr);
