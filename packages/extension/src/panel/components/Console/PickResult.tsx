...
export function PickResult({ data }: { data: PickResultData }) {
    ...
    return (
        <div data-type="pick-result">
            {/* ─── locator section ───────────────────────────────────── */}
            <div className="ot-row"><span className="ot-key">locator</span>{data.ref && <span className="text-(--text-dim) ml-1">[{data.ref}]</span>}</div>
                ...
                <div>
                    ...
                    {showDetails && (
                        <div className="ot-children">
                            ...
                            {d.value !== undefined && <div className="ot-row"><span className="ot-key">value</span><span className="ot-colon">:&nbsp;</span><span className="ot-string">"{d.value}"</span></div>}
                            {d.checked !== undefined && <div className="ot-row"><span className="ot-key">checked</span><span className="ot-colon">:&nbsp;</span><span className="ot-boolean">{String(d.checked)}</span></div>}
