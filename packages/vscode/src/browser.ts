...
export class BrowserManager {
  ...
  async launch(opts: LaunchOptions) {
    ...
    // 4. Wait for handleBridgeCommand
  ...
  // ─── Recording via Playwright's built-in Recorder ─────────────────────────
  // Uses context._enableRecorder() with recorderMode: 'api' — the same
  // infrastructure as `playwright codegen`. Locators are generated by
  // Playwright's injected selector engine (no normalize() needed).
  // Events flow in-process via the channel, no polling or WebSocket.
  ...
  async startRecording(callbacks: {
    onActionAdded: (code: string) => void;
    onActionUpdated: (code: string) => void;
  }): Promise<void> {
    ...
    await this._context._enableRecorder(
      { mode: 'recording', language: 'javascript', recorderMode: 'api' },
      {
        actionAdded: (_page: any, _action: any, code: string) => {
          callbacks.onActionAdded(code);
        },
        actionUpdated: (_page: any, _action: any, code: string) => {
          callbacks.onActionUpdated(code);
        },
      },
    );
  ...
  async stopRecording(): Promise<void> {
    ...
    await this._context._disableRecorder();
